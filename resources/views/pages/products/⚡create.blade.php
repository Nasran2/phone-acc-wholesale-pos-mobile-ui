...
<div>
    <section class="app-card p-4">
        <div class="flex flex-col gap-2">
            <p class="text-xs font-semibold uppercase tracking-wide text-zinc-400">Create</p>
            <h2 class="font-display text-xl font-semibold text-zinc-900">{{ __('New product') }}</h2>
            <p class="text-sm text-zinc-500">{{ __('Add phone accessories with pricing and stock details.') }}</p>
        </div>
    </section>

    <form wire:submit="save" class="mt-6">
        @include('pages.products.partials.form', ['submitLabel' => __('Save product')])
    </form>
</div>
